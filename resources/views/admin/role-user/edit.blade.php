...
<div class="content-wrapper">
    <section class="content">
        <div class="container-fluid">
            ...
            <div class="card">
                <form action="{{ route('admin.role-user.update', $ru->idrole_user) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="iduser" id="iduser" class="form-control @error('iduser') is-invalid @enderror" required>
                                ...
                                @foreach ($users as $u)
                                    <option value="{{ $u->iduser }}" {{ old('iduser', $ru->iduser) == $u->iduser ? 'selected' : '' }}>
                                        {{ $u->nama }} ({{ $u->email }})
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="idrole" id="idrole" class="form-control @error('idrole') is-invalid @enderror" required>
                                ...
                                @foreach ($roles as $r)
                                    <option value="{{ $r->idrole }}" {{ old('idrole', $ru->idrole) == $r->idrole ? 'selected' : '' }}>
                                        {{ $r->nama_role }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="status" id="status" class="form-control @error('status') is-invalid @enderror">
                                <option value="1" {{ old('status', $ru->status) == '1' ? 'selected' : '' }}>Aktif</option>
                                <option value="0" {{ old('status', $ru->status) == '0' ? 'selected' : '' }}>Nonaktif</option>
                            </select>
